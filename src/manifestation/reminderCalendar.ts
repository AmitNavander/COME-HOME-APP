export type ReminderInput = { start: string; daily: boolean; title: string; action: string; uid: string };
...
export function reminderCalendar(input: ReminderInput, now = new Date()): string {
  ...
  const lines = ['BEGIN:VCALENDAR', 'VERSION:2.0', 'PRODID:-//COME HOME//Foundation reminders//EN', 'CALSCALE:GREGORIAN', 'BEGIN:VEVENT', `UID:${input.uid}@come-home`, `DTSTAMP:${now.toISOString().replace(/[-:]/g, '').replace(/\.\d{3}/, '')}`, `DTSTART:${input.start.replace(/[-:]/g, '')}00`, 'DURATION:PT5M', ...(input.daily ? ['RRULE:FREQ=DAILY;COUNT=7'] : []), `SUMMARY:${escapeText('COME HOME · ' + input.title)}`, `DESCRIPTION:${escapeText(input.action + '\n\nPause, notice and take one small step. Open COME HOME → Manifest → Foundation to return to this day. Manage or delete this reminder in your calendar. This reminder stays with this practice; it does not advance to the next lesson.')}`, 'STATUS:CONFIRMED', 'TRANSP:TRANSPARENT', 'BEGIN:VALARM', 'ACTION:DISPLAY', 'TRIGGER:PT0S', `DESCRIPTION:${escapeText('COME HOME · ' + input.title)}`, 'END:VALARM', 'END:VEVENT', 'END:VCALENDAR'];
